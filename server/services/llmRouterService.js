...
async function selectLLM(query, context) {
  ...
  // PRIORITY 0: User-Specified Ollama Model (Bypass DB for local models)
  if (preferredProvider === 'ollama' && user?.ollamaModel) {
    console.log(`[LLMRouter] Decision: Using user-specified Ollama model '${user.ollamaModel}'.`);
    return {
      chosenModel: {
        provider: 'ollama',
        modelId: user.ollamaModel,
        strengths: ['general', 'local'] // Virtual strengths
      },
      logic: 'user_defined_local_model'
    };
  }
  ...
  return {
    chosenModel: { modelId: 'gemma-3-27b-it', provider: 'gemini' },
