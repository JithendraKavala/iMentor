...
const Section = ({ title, icon: Icon, children }) => (
    <div className="border border-border-light dark:border-border-dark rounded-lg overflow-hidden">
        <h3 className="flex items-center gap-2 p-3 bg-gray-50 dark:bg-white/5 border-b border-border-light dark:border-border-dark font-semibold">
            <Icon className="text-gray-500 dark:text-gray-400" size={18} /> {title}
        </h3>
...
const IntegrityReportPanel = ({ report, isLoading, error, steps, currentStep, onFindingSelect, onApplySuggestion }) => { // Add onApplySuggestion prop
        ...
        return (
            <div className="h-full flex flex-col justify-center items-center text-center p-4">
                ...
                <div className="w-full max-w-sm bg-gray-200 dark:bg-gray-700 rounded-full h-2 my-3">
                    <div className="bg-gray-900 dark:bg-gray-100 h-2 rounded-full transition-all duration-500" style={{ width: `${stepInfo.progress}%` }}></div>
                </div>
    ...
    return (
        <div className="h-full flex flex-col bg-surface-light dark:bg-chat-surface-dark border border-border-light dark:border-border-dark rounded-lg">
